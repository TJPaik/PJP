...
def plot_2dim(data, vert, xlabel='Principal Component 1', ylabel='Principal Component 2', fig_size=(10, 10), ax=None,
              pt_style=None):
...
def plot_eps(p1, vert, dist=None, type=None, vert_list=None, fig_size=(10, 10), ax=None, pt_style=None,
             scrollable=False):
    """
...
def plot_diagram_density(dgm, lognorm=True, diagonal=True,
   ...
   ----------
    ...
    hist2d, histx, histy, im = ax.hist2d([p[0] for p in dgm if p[0] != inf and p[1] != inf],
                                         [p[1] for p in dgm if p[0] != inf and p[1] != inf], norm=norm, **hist_kwargs)
